...
function addSurvey(survey) {
    return openDB().then(database => {
        return new Promise((resolve, reject) => {
            ...
            transaction.onerror = event => reject(event.target.error || new Error('IndexedDB transaction failed'));
            transaction.onabort = event => reject(event.target.error || new Error('IndexedDB transaction aborted'));
        });
    });
}
...
function deleteSurveyFromDB(id) {
    return openDB().then(database => {
        return new Promise((resolve, reject) => {
            const transaction = database.transaction([STORE_NAME, PENDING_STORE], 'readwrite');
            const store = transaction.objectStore(STORE_NAME);
...
/**
 * Cập nhật trạng thái đồng bộ của một khảo sát đã lưu.
 */
function updateSurveyStatus(id, syncStatus) {
    return openDB().then(database => {
        return new Promise((resolve, reject) => {
            const transaction = database.transaction([STORE_NAME], 'readwrite');
            const store = transaction.objectStore(STORE_NAME);
...
function addPendingSurvey(survey) {
    return openDB().then(database => {
        return new Promise((resolve, reject) => {
            ...
            transaction.onerror = e => reject(e.target.error || new Error('Pending queue transaction failed'));
            transaction.onabort = e => reject(e.target.error || new Error('Pending queue transaction aborted'));
        });
    });
}
